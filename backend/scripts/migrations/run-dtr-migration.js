...
async function runMigration() {
  ...
  try {
    ...
    if (isRollback) {
      console.log('⚠️  ROLLBACK MODE: This will remove all DTR tables and views!\n');
    } else {
    ...
    if (!isRollback) {
      ...
      const [views] = await connection.query("SHOW FULL TABLES WHERE Table_type = 'VIEW' AND Tables_in_employee_management_system LIKE 'v_dtr%'");
      console.log(`   ✅ Found ${views.length} DTR view(s)`);
      
      // Display table structures
      console.log('\n📋 Table Structures:\n');
      
      const [batchesDesc] = await connection.query('DESCRIBE dtr_import_batches');
      ...
      // Check indexes
      const [batchesIndexes] = await connection.query('SHOW INDEX FROM dtr_import_batches');
      const [recordsIndexes] = await connection.query('SHOW INDEX FROM dtr_records');
      ...
      if (tables.length === 0) {
        ...
        console.log(`   ⚠️  Warning: ${tables.length} DTR table(s) still exist`);
      }
    }
    
    console.log('\n========================================');
    console.log(isRollback ? '✅ Rollback completed successfully!' : '✅ Migration completed successfully!');
    console.log('========================================\n');
