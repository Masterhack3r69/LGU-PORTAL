...
const leaveTypeValidationRules = [
  body("name")
    ...
    .withMessage(
      "Leave type name is required and must be less than 50 characters"
    ),
  body("code")
    .trim()
    .isLength({ min: 1, max: 10 })
    .withMessage(
      "Leave type code is required and must be less than 10 characters"
    )
    .matches(/^[A-Z]+$/)
...
const getLeaveById = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    leave.employee_id !== currentUser.employee_id
  ) {
  ...
  try {
    ...
    if (hrUserIds.length > 0) {
      await notificationService.sendLeaveNotification(
        ...
        notificationService.notificationTypes.LEAVE_SUBMITTED,
        hrUserIds
      );
    }
  } catch (notificationError) {
    console.error(
      "Failed to send leave submission notification:",
      notificationError
    );
...
const updateLeave = asyncHandler(async (req, res) => {
  ...
  if (leave.status !== "Pending") {
    throw new ValidationError(
      "Cannot update leave application that has been reviewed"
    );
  }
...
const approveLeave = asyncHandler(async (req, res) => {
  ...
  if (currentUser.role !== "admin") {
    throw new ValidationError(
      "Only administrators can approve leave applications"
    );
  }
  ...
  try {
    ...
    console.log(`DEBUG: Looking for employee with ID: ${leave.employee_id}`);
    ...
    console.log(`DEBUG: Employee exists check:`, empCheck);
    ...
    console.log(`DEBUG: User relationship check:`, userCheck);
    ...
    console.log(`DEBUG: Notification query result:`, employeeRows);

    if (employeeRows.length > 0) {
      const employee = employeeRows[0];
      console.log(`DEBUG: Sending notification to user ID: ${employee.id}`);
      await notificationService.sendLeaveNotification(
        {
          id: leave.id,
        ...
        notificationService.notificationTypes.LEAVE_APPROVED,
        [employee.id]
      );
      console.log(`DEBUG: Notification sent successfully`);
    } else {
      console.log(`DEBUG: No active user found for employee ID: ${leave.employee_id}`);
    }
  } catch (notificationError) {
    console.error(
...
const rejectLeave = asyncHandler(async (req, res) => {
  ...
  if (currentUser.role !== "admin") {
    throw new ValidationError(
      "Only administrators can reject leave applications"
    );
  }

  if (!review_notes || review_notes.trim().length === 0) {
    throw new ValidationError(
      "Review notes are required when rejecting a leave application"
    );
  }
  ...
  try {
    ...
    // DEBUG: Log the employee_id we're trying to find
    console.log(`DEBUG: REJECT - Looking for employee with ID: ${leave.employee_id}`);
    ...
    console.log(`DEBUG: REJECT - Employee exists check:`, empCheck);
    ...
    console.log(`DEBUG: REJECT - User relationship check:`, userCheck);
    ...
    console.log(`DEBUG: REJECT - Notification query result:`, employeeRows);

    if (employeeRows.length > 0) {
      const employee = employeeRows[0];
      console.log(`DEBUG: REJECT - Sending notification to user ID: ${employee.id}`);
      await notificationService.sendLeaveNotification(
        {
          id: leave.id,
          employee_id: leave.employee_id,
        ...
        notificationService.notificationTypes.LEAVE_REJECTED,
        [employee.id]
      );
      console.log(`DEBUG: REJECT - Notification sent successfully`);
    } else {
      console.log(`DEBUG: REJECT - No active user found for employee ID: ${leave.employee_id}`);
    }
  } catch (notificationError) {
    console.error(
...
const createAdminLeave = asyncHandler(async (req, res) => {
  ...
  if (currentUser.role !== "admin") {
    throw new ValidationError(
      "Only administrators can create leave on behalf of employees"
    );
  }
  ...
  const approveResult = await savedLeave.approve(
    currentUser.id,
  ...
  if (!approveResult.success) {
    throw new Error(
      approveResult.error || "Failed to auto-approve leave application"
    );
  }
...
const getLeaveBalances = asyncHandler(async (req, res) => {
  ...
  console.log(
    `DEBUG: getLeaveBalances called with employeeId: ${employeeId}, year: ${year}`
  );
  console.log(
    `DEBUG: Current user role: ${currentUser.role}, employee_id: ${currentUser.employee_id}`
  );
  ...
  if (
    currentUser.role !== "admin" &&
    parseInt(employeeId) !== currentUser.employee_id
  ) {
    console.log(
      `DEBUG: Access denied - user ${currentUser.employee_id} trying to access employee ${employeeId}`
    );
    throw new ValidationError("Access denied");
  }
  ...
  try {
    ...
    if (!result.success) {
      console.error(
        `DEBUG: Error from LeaveBalance.getEmployeeBalances:`,
        result.error
      );
      throw new Error(result.error);
    }
...
const initializeLeaveBalances = asyncHandler(async (req, res) => {
  ...
  if (currentUser.role !== "admin") {
    throw new ValidationError(
      "Only administrators can initialize leave balances"
    );
  }
...
const processMonthlyAccrual = asyncHandler(async (req, res) => {
  ...
  const result = await LeaveBalance.processMonthlyAccrual(
    employee_id,
    year,
    month
  );
  ...
  if (!result.success) {
    ...
    if (
      result.message &&
      result.message.includes("no existing leave balances")
    ) {
...
const getAllLeaveTypes = asyncHandler(async (req, res) => {
  ...
  const filters = {};
  if (search) filters.search = search;
  if (is_monetizable !== undefined)
    filters.is_monetizable = is_monetizable === "true";
  if (requires_medical_certificate !== undefined)
    filters.requires_medical_certificate =
...
const createLeaveBalance = asyncHandler(async (req, res) => {
  ...
  if (!employee_id || !leave_type_id || !year) {
    throw new ValidationError(
      "Employee ID, leave type ID, and year are required"
    );
  }
...
const updateLeaveBalance = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { earned_days, used_days, monetized_days, carried_forward, reason } =
    req.body;
  const currentUser = req.session.user;
  ...
  const leaveBalance = new LeaveBalance({
    ...existingBalance,
    earned_days:
      earned_days !== undefined ? earned_days : existingBalance.earned_days,
    used_days: used_days !== undefined ? used_days : existingBalance.used_days,
    monetized_days:
      monetized_days !== undefined
        ? monetized_days
        : existingBalance.monetized_days,
    carried_forward:
      carried_forward !== undefined
        ? carried_forward
        : existingBalance.carried_forward,
  });
  ...
  await logManualBalanceUpdate(
    currentUser.id,
    existingBalance.employee_id,
    existingBalance.leave_type_id,
    existingBalance.year,
    "UPDATE",
    reason,
    req
  );
...
const deleteLeaveBalance = asyncHandler(async (req, res) => {
  ...
  await logManualBalanceUpdate(
    currentUser.id,
    balanceRecord.employee_id,
    balanceRecord.leave_type_id,
    balanceRecord.year,
    "DELETE",
    "Manual deletion",
    req
...
const logManualBalanceUpdate = async (
  userId,
  employeeId,
  leaveTypeId,
  year,
...
const processLeaveMonetization = asyncHandler(async (req, res) => {
  ...
  if (!employee_id || !leave_type_id || !year || !days_to_monetize) {
    throw new ValidationError(
      "Employee ID, leave type ID, year, and days to monetize are required"
    );
  }
  ...
  // Send notification to employee about leave monetization
  try {
    const { pool } = require("../config/database");

    // DEBUG: Log the employee_id we're trying to find
    console.log(`DEBUG: MONETIZATION - Looking for employee with ID: ${employee_id}`);
    ...
    console.log(`DEBUG: MONETIZATION - Employee exists check:`, empCheck);
    ...
    console.log(`DEBUG: MONETIZATION - User relationship check:`, userCheck);
    ...
    console.log(`DEBUG: MONETIZATION - Notification query result:`, employeeRows);

    if (employeeRows.length > 0) {
      const employee = employeeRows[0];
      console.log(`DEBUG: MONETIZATION - Sending notification to user ID: ${employee.id}`);

      // Get leave type name for the notification
      const leaveTypeResult = await LeaveType.findById(leave_type_id);
      const leaveTypeName = leaveTypeResult.success ? leaveTypeResult.data.name : "Leave";

      await notificationService.createNotification({
        user_id: employee.id,
        type: 'leave_monetized',
        title: 'Leave Balance Monetized',
        message: `Your ${leaveTypeName} balance has been monetized. ${days_to_monetize} days have been converted to monetary value for year ${year}.`,
        priority: 'HIGH',
        ...
        metadata: {
          ...
          days_monetized: days_to_monetize,
          year: year,
          monetization_amount: result.data.monetization_amount
        }
      });
      console.log(`DEBUG: MONETIZATION - Notification sent successfully`);
    } else {
      console.log(`DEBUG: MONETIZATION - No active user found for employee ID: ${employee_id}`);
    }
  } catch (notificationError) {
    console.error(
      "Failed to send leave monetization notification:",
      notificationError
    );
    // Don't fail the request if notification fails
  }
...
const processCarryForward = asyncHandler(async (req, res) => {
  ...
  if (!employee_id || !from_year || !to_year) {
    throw new ValidationError(
      "Employee ID, from year, and to year are required"
    );
  }

  if (to_year !== from_year + 1) {
    throw new ValidationError(
      "To year must be exactly one year after from year"
    );
  }

  const result = await LeaveBalance.processCarryForward(
    employee_id,
    from_year,
    to_year
  );
...
const getLeaveCalendar = asyncHandler(async (req, res) => {
  ...
  const result = await Leave.getLeaveCalendar(
    start_date,
    end_date,
    exclude_employee_id
  );
...
const generatePendingApprovalsDashboard = asyncHandler(async (req, res) => {
  ...
  if (!result.success) {
    throw new Error(
      result.error || "Failed to generate pending approvals dashboard"
    );
  }
