...
class CompensationBenefitController {
    ...
    static get validationRules() {
        return [
            ...
            body('benefit_type')
                .isIn(['TERMINAL_LEAVE', 'MONETIZATION', 'PBB', 'MID_YEAR_BONUS', 'YEAR_END_BONUS', 'EC', 'GSIS', 'LOYALTY'])
                ...
                case 'MONETIZATION':
                    if (!daysToMonetize) {
                        return res.status(400).json({
                            success: false,
                            error: 'Days to monetize is required for monetization calculation'
                        });
                    }
                    result = await this.service.calculateMonetization(employeeId, parseFloat(daysToMonetize));
                    break;
                case 'PBB':
    ...
    async bulkCalculate(req, res) {
        try {
            ...
            if (!benefitType || !Array.isArray(employeeIds) || employeeIds.length === 0) {
                return res.status(400).json({
                    success: false,
            ...
            const result = await this.service.bulkCalculateBenefit(benefitType, employeeIds, options);
    ...
    async processMonetization(req, res) {
        try {
            ...
            const processedBy = req.session.user.id;
            const result = await this.service.processMonetization(employee_id, days_to_monetize, processedBy, notes);
