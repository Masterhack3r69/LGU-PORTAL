...
import { Upload, FileText, Download, Trash2, AlertCircle, CheckCircle, Clock, X } from 'lucide-react';
...
export function DocumentUpload({ employeeId, documents, onDocumentUploaded }: DocumentUploadProps) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (selectedDocumentType) {
        const documentType = documentTypes.find(dt => dt.id.toString() === selectedDocumentType);
        if (documentType) {
  ...
  const handleDocumentTypeChange = (value: string) => {
    ...
    if (selectedFile) {
      const documentType = documentTypes.find(dt => dt.id.toString() === value);
      if (documentType) {
        const validation = documentService.validateFile(selectedFile, documentType);
        if (!validation.isValid) {
          setMessage({ type: 'error', text: validation.error || 'Invalid file' });
          setSelectedFile(null);
          // Reset file input
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Document uploaded successfully. It will be reviewed by an administrator.' });
      onDocumentUploaded();
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Upload and manage your documents. All uploads require admin approval.
              </CardDescription>
            </div>
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Select a document type and file to upload. All uploads require admin approval.
                  </DialogDescription>
                </DialogHeader>
                  ...
                  <div className="space-y-2">
                    ...
                    {selectedDocumentType && (
                      <div className="text-sm text-muted-foreground">
                        {(() => {
                          const docType = documentTypes.find(dt => dt.id.toString() === selectedDocumentType);
                          if (docType) {
                            return (
                              <div>
                                Max size: {formatFileSize(docType.max_file_size)}
                                {docType.allowed_extensions.length > 0 && (
                                  <span> • Allowed: {docType.allowed_extensions.join(', ')}</span>
                                )}
                              </div>
                            );
                          }
