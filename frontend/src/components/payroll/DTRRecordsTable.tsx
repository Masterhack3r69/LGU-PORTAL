...
export function DTRRecordsTable({
  periodId,
  periodDescription,
  onRecordUpdate,
}: DTRRecordsTableProps) {
  ...
  const [editingRecord, setEditingRecord] = useState<DTRRecordDetail | null>(
    null
  );
  ...
  const [deletingRecord, setDeletingRecord] = useState<DTRRecordDetail | null>(
    null
  );
  ...
  const handleEdit = (record: DTRRecordDetail) => {
    ...
    setEditWorkingDays(
      record.workingDays != null ? record.workingDays.toString() : "0"
    );
  ...
  const handleSaveEdit = async () => {
    ...
    if (isNaN(workingDays) || workingDays < 0) {
      showToast.error(
        "Invalid Input",
        "Working days must be a valid non-negative number"
      );
      return;
    }
    ...
    try {
      ...
      showToast.success(
        "Record Updated",
        "DTR record has been updated successfully"
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast.success(
        "Record Deleted",
        "DTR record has been deleted successfully"
      );
  ...
  return (
    <div className="space-y-4">
      {/* Sticky Header with Title, Description, and Search Bar */}
      <div className="sticky top-0 z-10 bg-background border-b">
        <div className="p-6 pb-4 space-y-3">
          <div>
            <h2 className="text-lg font-semibold">DTR Records</h2>
                ...
                <TableRow key={record.id}>
                  ...
                  <TableCell className="text-right font-medium">
                    {record.workingDays != null
                      ? Number(record.workingDays).toFixed(2)
                      : "0.00"}
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div>
                        <div className="text-sm">
                          {record.importedAt
                            ? dtrService.formatDate(record.importedAt)
                            : "N/A"}
                        </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{deletingRecord?.employeeName}</strong>? This action will
              mark the record as deleted and it will no longer be used in
